...
namespace MusicShop
{
    public class MusicShopBL
    {
        ...
        public static bool RegisterNewUser(string username, string pass, DateTime dob, string email, ref user newUser)
        {
            ...
            newUser = new user { userName = username, password = hashedPassword, dateOfBirth = dob, userEmail = email, accessLevel = "general", discountPoints = 0};
        ...
        public static bool InsertNewRating(rating rating)
        {
            try
            {
                using (var db = new musicShopEntities())
